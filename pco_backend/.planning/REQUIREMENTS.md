...
- [ ] **AUTH-01**: User can log in with email/password via POST /api/auth/login — returns JWT access token + refresh token (refresh token stored in DB)
- [ ] **AUTH-02**: User can refresh access token via POST /api/auth/refresh — exchange valid DB-stored refresh token for new access token
...
- [ ] **AUTH-06**: Deactivated users are rejected at login and token refresh
- [ ] **AUTH-07**: Passwords are hashed with bcrypt via passlib before storage
